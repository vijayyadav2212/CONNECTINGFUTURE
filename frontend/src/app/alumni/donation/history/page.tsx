...
const statusStyle = (s: string) => {
  switch (s?.toLowerCase()) {
    ...
    default: return 'bg-[#f6f3eb] text-gray-600 border-gray-200';
  }
};
      ...
      <div className="space-y-5">
        ...
        <div className="bg-teal-950 rounded-[32px] border border-teal-900/20 px-6 py-8 md:px-10 shadow-lg flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 relative overflow-hidden text-white">
          ...
          <div className="relative z-10 flex flex-col gap-2">
            <div className="flex items-center gap-2 text-teal-200 font-bold text-xs bg-[#f6f3eb]/10 w-fit px-3 py-1 rounded-full border border-white/15">
              <Heart className="w-3.5 h-3.5 text-rose-400 fill-rose-400" />
              <span>Giving Records</span>
            </div>
            <h1 className="text-3xl md:text-4xl font-extrabold text-white tracking-tight flex items-center gap-3">
              Donation History
            </h1>
            <p className="text-gray-300 text-sm md:text-base font-medium max-w-xl">
              Track contributions and see the collective impact of our alumni community.
            </p>
          </div>
          <button onClick={() => router.push('/alumni/donation')} className="relative z-10 flex items-center gap-2 px-5 py-3 bg-white text-sm font-bold text-teal-950 rounded-2xl border border-teal-50 shadow-md hover:bg-[#f6f3eb] hover:scale-[1.02] transition-all shrink-0">
            <ArrowLeft className="w-4 h-4 text-teal-950" />Back to Donation
          </button>
        </div>
        ...
        {analytics && (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-3">
            {[
              { label: 'Total Donated', value: formatCurrency(analytics.total_donated), icon: <TrendingUp className="w-4 h-4 text-teal-600" />, bg: 'bg-teal-50', color: 'text-teal-700' },
              ...
              { label: 'Avg Donation', value: formatCurrency(analytics.avg_donation), icon: <IndianRupee className="w-4 h-4 text-teal-600" />, bg: 'bg-teal-50', color: 'text-teal-700' },
              ...
              { label: 'Recent (30d)', value: String(analytics.recent_donations), icon: <Calendar className="w-4 h-4 text-pink-600" />, bg: 'bg-[#f6f3eb]', color: 'text-pink-700' },
            ].map((s, i) => (
        ...
        <div className="bg-white rounded-[24px] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-4">
          <div className="flex items-center gap-2 mb-3">
            <Filter className="w-4 h-4 text-teal-950" />
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Select value={statusFilter} onValueChange={handleStatusChange}>
              ...
              <SelectContent className="bg-white border-slate-200 rounded-xl text-slate-900 shadow-xl">
                <SelectItem className="text-slate-800 focus:text-slate-900 focus:bg-[#f6f3eb] cursor-pointer" value="all">All Status</SelectItem>
                <SelectItem className="text-slate-800 focus:text-slate-900 focus:bg-[#f6f3eb] cursor-pointer" value="completed">Completed</SelectItem>
                <SelectItem className="text-slate-800 focus:text-slate-900 focus:bg-[#f6f3eb] cursor-pointer" value="pending">Pending</SelectItem>
                <SelectItem className="text-slate-800 focus:text-slate-900 focus:bg-[#f6f3eb] cursor-pointer" value="failed">Failed</SelectItem>
              </SelectContent>
            </Select>
            <button onClick={handleSearch} className="flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-xl bg-teal-950 text-white hover:bg-teal-900 transition-colors shadow-sm">
              <Search className="w-4 h-4" />Search
            </button>
            {user?.email && (
              <button onClick={handleMyDonations} className="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-xl bg-white border border-teal-200 text-teal-700 hover:bg-teal-50 transition-colors">
        ...
        <div className="bg-white rounded-[24px] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
          <div className="flex items-center gap-2 px-5 py-4 border-b border-slate-100 bg-[#f6f3eb]/60">
            <Gift className="w-4 h-4 text-teal-950" />
          ...
          <div className="p-4">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-14 text-center">
                <div className="w-12 h-12 rounded-full bg-[#f6f3eb] flex items-center justify-center mb-3"><Heart className="w-6 h-6 text-slate-300" /></div>
                  ...
                  <div key={d.id} className="rounded-[18px] border border-slate-100 hover:shadow-md transition-all p-4">
                    <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div className="w-10 h-10 rounded-full bg-teal-100 text-teal-700 text-sm font-bold flex items-center justify-center shrink-0">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {d.message && (
                            <div className="bg-teal-50 border-l-2 border-teal-400 rounded-r-lg px-3 py-2 mb-2">
                              <p className="text-xs text-teal-700 italic">"{d.message}"</p>
                            </div>
                          )}
                          <div className="flex flex-wrap gap-2 text-[11px] text-slate-500">
                            <span className="flex items-center gap-1 bg-[#f6f3eb] rounded-full px-2 py-0.5"><Calendar className="w-3 h-3" />{formatDate(d.created_at)}</span>
                            <span className="bg-[#f6f3eb] rounded-full px-2 py-0.5">{d.payment_method}</span>
                            {d.cause_category && <span className="bg-[#f6f3eb] rounded-full px-2 py-0.5">{d.cause_category}</span>}
                            <span className="bg-[#f6f3eb] rounded-full px-2 py-0.5">{d.donation_type}</span>
                          </div>
                        </div>
                      </div>
            ...
            {totalPages > 1 && (
              <div className="flex flex-wrap items-center justify-center gap-3 mt-5 pt-4 border-t border-slate-100">
                <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1 || loading} className="flex items-center gap-1.5 px-4 py-2 text-sm font-semibold rounded-xl border border-slate-200 text-slate-700 hover:bg-[#f6f3eb] disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                  <ArrowLeft className="w-4 h-4" />Previous
                </button>
                <span className="px-4 py-2 text-sm font-bold text-slate-700 bg-[#f6f3eb] rounded-xl border border-slate-200">
                  {currentPage} / {totalPages}
                </span>
                <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages || loading} className="flex items-center gap-1.5 px-4 py-2 text-sm font-semibold rounded-xl border border-slate-200 text-slate-700 hover:bg-[#f6f3eb] disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
