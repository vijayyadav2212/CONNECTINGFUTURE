  ...
  return (
    <>
      <div className="space-y-6">
        ...
        {/* Page Header */}
        <div className="bg-teal-950  rounded-[32px] px-8 py-10 md:px-12 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 relative overflow-hidden text-white">
        <svg className="absolute right-0 bottom-0 w-[300px] h-full pointer-events-none opacity-50" viewBox="0 0 200 100" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="relative z-10 flex flex-col gap-3">
            ...
            <p className="text-gray-300 font-semibold text-[16px]">Your community is growing. Ready to connect and make an impact today?</p>
          </div>
          <button
            onClick={() => setShowForm(true)}
            className="relative z-10 flex items-center justify-center gap-2 p-5 bg-white text-teal-950 hover:text-teal-950 font-bold rounded-[20px] transition-all shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] hover:scale-105 border border-teal-50"
            title="Create Event"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
          {[
            { label: 'Total Events', value: events.length, color: 'text-slate-800', bg: 'bg-white', iconBg: 'bg-[#f6f3eb] border border-slate-100' },
            { label: 'Upcoming', value: upcomingEvents.length, color: 'text-slate-800', bg: 'bg-white', iconBg: 'bg-[#f6f3eb] border border-slate-100' },
            { label: 'Past Events', value: pastEvents.length, color: 'text-slate-800', bg: 'bg-white', iconBg: 'bg-[#f6f3eb] border border-slate-100' },
            { label: 'Online', value: events.filter(e => e.is_virtual).length, color: 'text-slate-800', bg: 'bg-white', iconBg: 'bg-[#f6f3eb] border border-slate-100' },
          ].map((s, i) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-[32px] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-8 md:p-10">
            <div className="flex items-center gap-3 mb-8">
              <div className="w-2.5 h-2.5 rounded-full bg-[#f6f3eb]0 animate-pulse shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
            ...
            {upcomingEvents.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center bg-[#f6f3eb] rounded-[24px] border border-slate-100">
              ...
              <div className="space-y-4">
                {upcomingEvents.map((event, idx) => {
                  const colors = [
                    { badge: 'bg-[#f6f3eb] text-emerald-700 border-teal-900/10' },
                    ...
                    <div key={event.id} className="rounded-[24px] border border-slate-100 bg-white hover:shadow-xl transition-all duration-300 p-6 group relative overflow-hidden">
                      ...
                      <div className="flex items-start justify-between gap-4 mb-3">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <button onClick={() => handleShare(event)} className="p-2 text-slate-400 hover:text-slate-700 hover:bg-[#f6f3eb] rounded-xl transition-colors" title="Share">
                      ...
                      <div className="flex flex-wrap items-center gap-4 text-[12px] font-bold text-slate-400 pt-5 border-t border-slate-100">
                        ...
                        {event.is_virtual && event.location.startsWith('http') && (
                          <a href={event.location} target="_blank" rel="noreferrer" className="ml-auto px-5 py-2.5 bg-[#f6f3eb]0 text-white rounded-xl text-[12px] font-black hover:bg-[#f3b13a] transition-colors shadow-lg shadow-emerald-500/20">Join →</a>
                        )}
                      </div>
                    </div>
                  );
          ...
          <div className="bg-white rounded-[32px] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-8 md:p-10">
            ...
            <div className="space-y-4">
              ...
              {[
                { title: 'Career Fair 2024', date: 'November 10, 2024', type: 'Career', color: 'bg-teal-50 text-teal-700 border-teal-200', desc: '120+ attendees, 15 companies, 30+ job offers', attended: true },
                ...
                <div key={i} className="rounded-[20px] border border-slate-100 bg-[#f6f3eb] p-6">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-slate-200/50">
                    ...
                    <span className={`text-[12px] font-black uppercase tracking-widest px-3 py-1 rounded-lg ${e.attended ? 'bg-emerald-100/50 text-teal-950' : 'bg-slate-200/50 text-slate-500'}`}>
              ...
              {pastEvents.map((event, idx) => (
                <div key={event.id} className="rounded-[20px] border border-slate-100 bg-[#f6f3eb] p-6">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <h4 className="font-black text-slate-800 text-[15px]">{event.title}</h4>
                    <span className="text-[10px] font-black px-3 py-1 rounded-full border bg-[#f6f3eb] text-slate-600 border-slate-200 uppercase tracking-widest shrink-0">{event.type}</span>
                  </div>
      ...
      {/* Success Modal */}
      {showSuccessModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-teal-950/50">
