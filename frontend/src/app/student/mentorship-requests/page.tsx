    ...
    <StudentNavigation>
      <div className="min-h-screen relative overflow-hidden bg-gradient-to-br from-blue-50 via-green-50 to-orange-50">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          ></motion.div>
          <motion.div
            className="absolute top-0 -right-4 w-72 h-72 bg-orange-300 rounded-full mix-blend-multiply filter blur-xl opacity-30"
            animate={{
              x: [0, -100, 0],
              y: [0, 100, 0],
            }}
        ...
        <div className="p-4 sm:p-6 lg:p-8 relative">
          <div className="max-w-7xl mx-auto">
            ...
            <motion.div
              ...
              className="mb-8"
            >
              <div className="relative bg-gradient-to-br from-blue-100/60 via-green-100/50 to-orange-100/40 backdrop-blur-lg rounded-3xl p-8 md:p-10 shadow-xl border border-white/30 overflow-hidden">
                <div className="absolute inset-0 bg-white/20 backdrop-blur-sm rounded-3xl"></div>
                <div className="relative z-10 flex items-center justify-between">
                  <div>
                    <div className="flex items-center space-x-3 mb-3">
                      <span className="text-4xl">🎯</span>
                      <h1 className="text-3xl md:text-4xl font-bold text-gray-900">Find Your Mentor</h1>
                    </div>
            ...
            <motion.div
              ...
              className="mb-8"
            >
              <div className="bg-gradient-to-br from-white/70 via-white/60 to-white/50 backdrop-blur-lg rounded-3xl p-6 shadow-xl border border-white/30">
                <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                  <Input placeholder="Search skills/topics" value={q} onChange={(e) => setQ(e.target.value)} className="border-white/50 bg-white/60 backdrop-blur-sm" />
                  <Input placeholder="Min experience (years)" type="number" value={minExp as any} onChange={(e) => setMinExp(e.target.value ? Number(e.target.value) : "")} className="border-white/50 bg-white/60 backdrop-blur-sm" />
                  <Input placeholder="Max price (INR)" type="number" value={maxPrice as any} onChange={(e) => setMaxPrice(e.target.value ? Number(e.target.value) : "")} className="border-white/50 bg-white/60 backdrop-blur-sm" />
                  <Input placeholder="Min rating (1-5)" type="number" value={minRating as any} onChange={(e) => setMinRating(e.target.value ? Number(e.target.value) : "")} className="border-white/50 bg-white/60 backdrop-blur-sm" />
                  <div className="flex gap-2 justify-end">
                    <Button variant="outline" onClick={clearFilters} disabled={loading} className="backdrop-blur-sm border-2">Clear</Button>
                    <Button onClick={loadMentors} disabled={loading} className="bg-gradient-to-r from-green-500 to-blue-600 hover:from-green-600 hover:to-blue-700 text-white px-6">{loading ? "Searching..." : "Search"}</Button>
                  </div>
                </div>
