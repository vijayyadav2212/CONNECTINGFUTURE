...
const statusBadge = (status: string) => {
  switch (status) {
    ...
    case 'approved': return <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold bg-teal-950/10 text-red-700 border border-teal-200"><CheckCircle className="w-3 h-3" />Approved</span>;
  ...
  return (
    <>
      <div className="space-y-5">
        ...
        <div className="bg-teal-950/10 rounded-[20px] p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between border border-teal-900/10 gap-5">
          <div>
            ...
            <h1 className="text-3xl sm:text-4xl font-extrabold text-teal-950 tracking-tight mb-2 flex items-center gap-2">Alumni Approvals</h1>
            <p className="text-teal-800 text-[15px] sm:text-base">Review and approve alumni registration requests</p>
            <div className={`mt-4 inline-flex items-center gap-3 rounded-2xl px-3.5 py-2.5 border shadow-sm transition-colors ${autoApproveEnabled ? 'bg-[#f6f3eb] border-emerald-200' : 'bg-white/80 border-teal-900/10'}`}>
              <Switch
                checked={autoApproveEnabled}
                onCheckedChange={handleAutoApproveToggle}
                disabled={autoApproveLoading || autoApproveSaving}
                className="data-[state=checked]:bg-[#f3b13a] data-[state=unchecked]:bg-slate-300"
                aria-label="Auto approve alumni registrations"
              />
              <div>
                <p className={`text-xs font-extrabold tracking-wide ${autoApproveEnabled ? 'text-emerald-800' : 'text-teal-950'}`}>Auto Approve Alumni</p>
                <p className={`text-[11px] ${autoApproveEnabled ? 'text-emerald-700' : 'text-teal-700'}`}>
              ...
              {(autoApproveLoading || autoApproveSaving) ? (
                ...
                <span className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full border ${autoApproveEnabled ? 'bg-emerald-100 text-emerald-700 border-emerald-300' : 'bg-[#f6f3eb] text-slate-600 border-slate-200'}`}>
          ...
          <div className="flex gap-3 shrink-0">
            {[
              { label: 'Pending', value: counts.pending, color: 'text-amber-700', bg: 'bg-white/60 border-teal-950/100 shadow-sm' },
              { label: 'Approved', value: counts.approved, color: 'text-red-600', bg: 'bg-white/60 border-teal-950/100 shadow-sm' },
            ].map(s => (
        ...
        <div className="bg-white rounded-2xl border border-teal-900/10 shadow-sm p-4 flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          <div className="relative flex-1 max-w-sm">
            ...
            <input type="text" placeholder="Search by name, email or major…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2.5 text-sm rounded-xl border border-teal-900/10 bg-white text-teal-950 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-200 focus:border-teal-400" />
          </div>
          <div className="flex items-center gap-1.5 flex-wrap">
            <Filter className="w-4 h-4 text-teal-600 shrink-0" />
            {filterBtns.map(b => (
              <button key={b.key} onClick={() => setFilter(b.key)} className={`px-3 py-1.5 text-xs font-bold rounded-xl transition-colors ${filter === b.key ? b.activeColor : 'bg-[#f6f3eb] text-teal-800 hover:bg-teal-100'}`}>{b.label}</button>
            ))}
          </div>
        </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-14 bg-white rounded-2xl border border-teal-900/10">
            ...
            <p className="text-sm text-teal-600">Loading requests…</p>
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-14 bg-white rounded-2xl border border-teal-900/10 text-center">
            <div className="w-12 h-12 rounded-full bg-[#f6f3eb] flex items-center justify-center mb-3"><UserCheck className="w-6 h-6 text-gray-300" /></div>
            <p className="font-bold text-teal-950 text-sm">No alumni found</p>
            <p className="text-xs text-teal-600 mt-1">Try adjusting your filters or search</p>
          </div>
        ) : (
          <div className="space-y-3">
            {filteredAlumni.map(alumni => (
              <div key={alumni.id} className="bg-white rounded-2xl border border-teal-900/10 shadow-sm p-4 hover:shadow-md transition-shadow">
                <div className="flex items-start gap-4">
                  {/* Avatar */}
                  <div className="w-10 h-10 rounded-full bg-teal-100 text-red-700 text-sm font-black flex items-center justify-center shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      <p className="font-bold text-teal-950 text-sm">{alumni.name}</p>
                      {statusBadge(alumni.status)}
                    </div>
                    <div className="flex flex-wrap gap-3 text-[11px] text-teal-700 mb-2">
                    ...
                    <div className="flex flex-wrap gap-3 text-[11px] mb-2">
                      <a href={`mailto:${alumni.email}`} className="flex items-center gap-1 text-teal-900 hover:underline"><Mail className="w-3 h-3" />{alumni.email}</a>
                      {alumni.phone && <span className="flex items-center gap-1 text-teal-700"><Phone className="w-3 h-3" />{alumni.phone}</span>}
                      {alumni.linkedinUrl && <a href={alumni.linkedinUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-teal-900 hover:underline"><Linkedin className="w-3 h-3" />LinkedIn</a>}
                    </div>
                    {alumni.bio && <p className="text-xs text-teal-700 line-clamp-2 mb-2">{alumni.bio}</p>}
                    {alumni.skills && alumni.skills.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {alumni.skills.slice(0, 6).map((sk, i) => <span key={i} className="px-2 py-0.5 bg-teal-950/10 text-red-700 border border-teal-200 rounded-full text-[10px] font-semibold">{sk}</span>)}
                      </div>
                    )}
                    <p className="text-[10px] text-teal-600">Submitted {alumni.submittedAt ? new Date(alumni.submittedAt).toLocaleDateString() : '—'}</p>
                  </div>

                  {/* Actions */}
                  {alumni.status === 'pending' && (
                    <div className="flex flex-col gap-1.5 shrink-0">
                      <button onClick={() => setSelectedAlumni(alumni)} className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-xl bg-[#f6f3eb] border border-teal-900/10 text-teal-800 hover:bg-[#f6f3eb] transition-colors">
      ...
      {selectedAlumni && !showRejectionModal && (
        <div className="fixed inset-0 bg-teal-950/40 flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar p-6 sm:p-8">
            <div className="flex items-start justify-between mb-8">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-teal-100 text-red-700 text-2xl font-black flex items-center justify-center shrink-0">
                  {selectedAlumni.name.split(' ').map((n: string) => n[0]).join('').slice(0, 2)}
                </div>
                <div>
                  <h3 className="text-2xl font-bold text-teal-950 leading-tight flex items-center gap-2">
                    {selectedAlumni.name}
                    {statusBadge(selectedAlumni.status)}
                  </h3>
                  <p className="text-teal-700 text-sm mt-1">{selectedAlumni.email}</p>
                </div>
              </div>
              <button onClick={() => setSelectedAlumni(null)} className="p-2 text-teal-600 hover:text-teal-800 hover:bg-[#f6f3eb] rounded-full transition-colors shrink-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-8">
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-base font-semibold text-teal-950">{selectedAlumni.major}</p>
                  <p className="text-sm text-teal-800">Class of {selectedAlumni.graduationYear}</p>
                </div>
                <div>
                  ...
                  <p className="text-base font-semibold text-teal-950">{selectedAlumni.jobTitle || 'Not specified'}</p>
                  <p className="text-sm text-teal-800">{selectedAlumni.company || ''}</p>
                </div>
              </div>
              <div className="space-y-6">
                <div>
                  ...
                  <div className="space-y-2.5">
                    {selectedAlumni.phone && <p className="text-sm text-teal-950 flex items-center gap-2"><Phone className="w-4 h-4 text-teal-600 shrink-0" /> {selectedAlumni.phone}</p>}
                    {selectedAlumni.location && <p className="text-sm text-teal-950 flex items-center gap-2"><MapPin className="w-4 h-4 text-teal-600 shrink-0" /> {selectedAlumni.location}</p>}
                  </div>
                </div>
                {selectedAlumni.linkedinUrl && (
                  <div>
                    ...
                    <a href={selectedAlumni.linkedinUrl} target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-teal-900 hover:text-teal-700 hover:underline flex items-center gap-1.5">
                      LinkedIn Profile
                    </a>
                  </div>
                )}
            ...
            {selectedAlumni.bio && (
              <div className="mb-8">
                ...
                <div className="bg-teal-950/10/50 p-4 rounded-2xl text-sm text-teal-900 leading-relaxed border border-teal-900/10/50">
                  {selectedAlumni.bio}
                </div>
              </div>
            )}
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedAlumni.skills.map((sk: string, i: number) => (
                    <span key={i} className="px-3 py-1 bg-teal-950/10 text-red-700 border border-teal-200 rounded-full text-xs font-semibold">{sk}</span>
                  ))}
                </div>
              </div>
            )}

            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-6 border-t border-teal-900/10 mt-8">
              <span className="text-xs font-semibold text-teal-600 tracking-wide uppercase">Submitted {new Date(selectedAlumni.submittedAt).toLocaleDateString()}</span>
      ...
      {showRejectionModal && selectedAlumni && (
        <div className="fixed inset-0 bg-teal-950/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <h3 className="text-base font-bold text-teal-950 mb-1">Reject Alumni Application</h3>
            <p className="text-sm text-teal-700 mb-4">You are rejecting <strong>{selectedAlumni.name}</strong>'s application. Please provide a reason:</p>
            <textarea value={rejectionReason} onChange={e => setRejectionReason(e.target.value)} placeholder="Enter rejection reason…" rows={4} className="w-full px-3 py-2.5 text-sm rounded-xl border border-teal-900/10 bg-white text-teal-950 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-red-200 focus:border-red-400 resize-none" />
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => { setShowRejectionModal(false); setRejectionReason(''); }} className="px-4 py-2 text-sm font-semibold rounded-xl border border-teal-900/10 text-teal-900 hover:bg-[#f6f3eb] transition-colors">Cancel</button>
