    ...
    <AdminNavigation>
      <div className="space-y-5">
        ...
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-2xl border border-green-100 p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><Briefcase className="w-6 h-6 text-green-600" />Job Management</h1>
            <p className="text-gray-500 text-sm mt-1">Manage job postings and internship opportunities</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <Link href="/admin/jobs/create" className="flex items-center gap-2 px-4 py-2.5 text-sm font-bold rounded-xl bg-green-600 text-white hover:bg-green-700 transition-colors shadow-sm">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          <div className="relative flex-1 max-w-sm">
            ...
            <input type="text" placeholder="Search by title, company, or location…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2.5 text-sm rounded-xl border border-gray-200 bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-200 focus:border-green-400" />
          </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex flex-col gap-1.5 shrink-0">
                      ...
                      {approvalStatus === 'pending' && (
                        <>
                          <button onClick={() => handleApprove(job.id)} className="flex items-center gap-1 px-3 py-1.5 text-xs font-semibold rounded-xl bg-green-600 text-white hover:bg-green-700 transition-colors"><CheckCircle className="w-3 h-3" />Approve</button>
