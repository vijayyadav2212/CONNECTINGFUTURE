    ...
    <AdminNavigation>
      <div className="max-w-3xl mx-auto space-y-5">
        ...
        <div className="bg-pink-50 rounded-[20px] p-6 sm:p-8 flex items-center gap-5 border border-pink-100">
          <Link href="/admin/jobs" className="w-10 h-10 rounded-xl bg-white/60 border border-white text-gray-600 hover:bg-white transition-all shadow-sm shrink-0 flex items-center justify-center">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-gray-900 tracking-tight flex items-center gap-2 mb-1">
              <Briefcase className="w-7 h-7 text-rose-500" />Post New Job
            </h1>
            <p className="text-gray-600 text-[15px] sm:text-base">Create a new job posting or internship opportunity</p>
          </div>
        </div>
        ...
        {showSuccess && (
          <div className="bg-pink-50 border border-pink-200 rounded-xl p-4 flex items-center gap-3">
            ...
            <div><p className="text-sm font-bold text-rose-800">Job posted successfully!</p><p className="text-xs text-rose-600">Redirecting…</p></div>
          </div>
        )}
            ...
            {steps.map((step, i) => (
              <React.Fragment key={step.id}>
                <div className="flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-black transition-colors ${activeStep >= step.id ? 'bg-rose-600 text-white' : 'bg-gray-100 text-gray-400'}`}>{step.icon}</div>
                  <span className={`hidden sm:block text-xs font-bold ${activeStep >= step.id ? 'text-gray-900' : 'text-gray-400'}`}>{step.name}</span>
                </div>
                {i < steps.length - 1 && <div className={`flex-1 h-0.5 mx-2 ${activeStep > step.id ? 'bg-rose-500' : 'bg-gray-200'}`} />}
              </React.Fragment>
            ))}
          </div>
        </div>
          ...
          {activeStep === 1 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="flex justify-end mt-5 pt-4 border-t border-gray-100">
                <button type="button" onClick={() => setActiveStep(2)} className="px-5 py-2.5 text-sm font-bold rounded-xl bg-rose-600 text-white hover:bg-rose-700 transition-colors">Next: Job Details →</button>
              </div>
            </div>
          )}
          ...
          {activeStep === 2 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="flex justify-between mt-5 pt-4 border-t border-gray-100">
                ...
                <button type="button" onClick={() => setActiveStep(3)} className="px-5 py-2.5 text-sm font-bold rounded-xl bg-rose-600 text-white hover:bg-rose-700 transition-colors">Next: Compensation →</button>
              </div>
            </div>
          )}
          ...
          {activeStep === 3 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              {formData.salaryMin && formData.salaryMax && (
                <div className="mt-4 bg-pink-50 border border-pink-200 rounded-xl p-4 flex items-center gap-3">
              ...
              <div className="flex justify-between mt-5 pt-4 border-t border-gray-100">
                ...
                <button type="button" onClick={() => setActiveStep(4)} className="px-5 py-2.5 text-sm font-bold rounded-xl bg-rose-600 text-white hover:bg-rose-700 transition-colors">Next: Application →</button>
              </div>
            </div>
          )}
          ...
          {activeStep === 4 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="flex justify-between mt-5 pt-4 border-t border-gray-100">
                ...
                <button type="submit" disabled={!canSubmit} className="flex items-center gap-2 px-6 py-2.5 text-sm font-bold rounded-xl bg-rose-600 text-white hover:bg-rose-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-sm">
