  ...
  return (
    <AdminNavigation>
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="bg-[#f6f3eb] rounded-[20px] p-6 sm:p-8 flex items-center gap-5 border border-teal-900/10">
          ...
          <div>
            <h1 className="text-3xl sm:text-4xl font-extrabold text-teal-950 tracking-tight flex items-center gap-2 mb-1">
              <Calendar className="w-7 h-7 text-teal-950" />Create Event
            </h1>
            <p className="text-teal-800 text-[15px]">Add a new event for students to discover</p>
          </div>
        </div>

        {/* Message */}
        {message && (
          <div className={`p-4 rounded-xl border flex items-center gap-3 text-sm font-semibold ${message.type === 'success' ? 'bg-[#f6f3eb] border-pink-200 text-rose-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
        ...
        {/* Form */}
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-teal-900/10 shadow-sm p-5 space-y-5">

          {/* Section: Basic */}
          <div>
            <div className="flex items-center gap-2 mb-3 pb-2 border-b border-teal-900/10">
              <Calendar className="w-4 h-4 text-teal-950" />
              <p className="text-xs font-black text-teal-900 uppercase tracking-wider">Event Details</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div><label className={labelCls}>Organizer</label>
                <div className="relative"><User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-teal-600" /><input type="text" name="organizer" value={formData.organizer} onChange={handleChange} placeholder="e.g., Tech Team" className={iconInputCls} /></div>
              </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3 pb-2 border-b border-teal-900/10">
              <Clock className="w-4 h-4 text-teal-950" />
              <p className="text-xs font-black text-teal-900 uppercase tracking-wider">Schedule & Location</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div><label className={labelCls}>Location *</label>
                <div className="relative"><MapPin className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-teal-600" /><input type="text" name="location" value={formData.location} onChange={handleChange} required placeholder="e.g., Tech Hub or Online" className={iconInputCls} /></div>
              </div>
              <div className="sm:col-span-2 flex items-center gap-3 bg-[#f6f3eb] border border-pink-200 rounded-xl p-3">
                <input type="checkbox" name="is_virtual" id="is_virtual" checked={formData.is_virtual} onChange={handleChange} className="w-4 h-4 text-teal-950 rounded border-gray-300 focus:ring-rose-500" />
                <label htmlFor="is_virtual" className="text-sm font-semibold text-teal-900 cursor-pointer">Virtual Event</label>
              </div>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3 pb-2 border-b border-teal-900/10">
              ...
              <p className="text-xs font-black text-teal-900 uppercase tracking-wider">Capacity & Pricing</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div><label className={labelCls}>Max Attendees</label>
                <div className="relative"><Users className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-teal-600" /><input type="number" name="max_attendees" value={formData.max_attendees} onChange={handleChange} placeholder="100" className={iconInputCls} /></div>
              </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3 pb-2 border-b border-teal-900/10">
              <Tag className="w-4 h-4 text-teal-950" />
              <p className="text-xs font-black text-teal-900 uppercase tracking-wider">Tags</p>
            </div>
          ...
          {/* Submit */}
          <div className="flex gap-3 pt-2 border-t border-teal-900/10">
            <Link href="/admin/events" className="px-5 py-2.5 text-sm font-bold rounded-xl border border-teal-900/10 text-teal-900 hover:bg-[#f6f3eb] transition-colors">Cancel</Link>
            <button type="submit" disabled={loading} className="flex items-center gap-2 px-6 py-2.5 text-sm font-bold rounded-xl bg-[#f3b13a] text-teal-950 font-bold hover:bg-[#d89c30] disabled:opacity-60 transition-colors shadow-sm">
