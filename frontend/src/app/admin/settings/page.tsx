...
// ─── Helpers ─────────────────────────────────────────────────────────────────
const inputCls = "w-full px-3 py-2.5 text-sm rounded-xl border border-teal-900/10 bg-white text-teal-950 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-200 focus:border-emerald-400";
const iconInputCls = (icon: boolean) => `w-full ${icon ? 'pl-9' : 'px-3'} pr-3 py-2.5 text-sm rounded-xl border border-teal-900/10 bg-white text-teal-950 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-200 focus:border-emerald-400`;
...
const InputField = ({ label, value, onChange, type = 'text', placeholder = '', icon, readOnly }: any) => (
  <div>
    ...
    <div className="relative">
      {icon && <div className="absolute left-3 top-1/2 -translate-y-1/2 text-teal-600">{icon}</div>}
...
const Toggle = ({ checked, onChange, label, description, setDirty }: any) => (
  <div className="flex items-center justify-between p-3.5 bg-[#f6f3eb] rounded-xl hover:bg-[#f6f3eb] transition-colors">
    <div className="flex-1 mr-4">
      <p className="text-xs font-bold text-teal-950">{label}</p>
      {description && <p className="text-[11px] text-teal-700 mt-0.5">{description}</p>}
    </div>
    <button type="button" onClick={() => { onChange(!checked); setDirty?.(); }}
      className={`relative w-10 h-5 rounded-full transition-colors shrink-0 ${checked ? 'bg-[#f3b13a]' : 'bg-gray-300'}`}>
...
const SectionHead = ({ icon, title, desc, iconBg }: any) => (
  <div className="flex items-center justify-between pb-3 mb-4 border-b border-teal-900/10">
      ...
      <div className="space-y-5">
        ...
        <div className="bg-[#f6f3eb] rounded-[20px] p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center sm:justify-between border border-teal-900/10 gap-5">
          <div>
            ...
            <h1 className="text-3xl sm:text-4xl font-extrabold text-teal-950 tracking-tight mb-2">Settings</h1>
            <p className="text-teal-800 text-[15px] sm:text-base">Manage your platform configuration and preferences</p>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={handleReset} className="flex items-center gap-1.5 px-4 py-2 text-sm font-bold rounded-xl border border-teal-900/10 text-teal-900 bg-white hover:bg-[#f6f3eb] transition-colors"><RefreshCw className="w-3.5 h-3.5" />Reset</button>
            <button onClick={handleSave} disabled={!unsaved}
              className={`flex items-center gap-1.5 px-4 py-2 text-sm font-bold rounded-xl transition-colors shadow-sm ${unsaved ? 'bg-[#f3b13a] text-teal-950 font-bold hover:bg-[#d89c30]' : 'bg-[#f6f3eb] text-teal-600 cursor-not-allowed'}`}>
        ...
        {/* Alerts */}
        {saved && (
          <div className="flex items-center gap-3 p-4 bg-[#f6f3eb] border border-emerald-200 rounded-xl text-sm font-semibold text-emerald-700">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[220px_1fr] gap-4">
          ...
          {/* Sidebar */}
          <div className="bg-white rounded-2xl border border-teal-900/10 shadow-sm p-2 sticky top-6 h-fit">
            {tabs.map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)}
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl mb-0.5 transition-colors text-left ${activeTab===t.id ? 'bg-[#f6f3eb] text-emerald-700 border border-teal-900/10' : 'text-teal-900 hover:bg-[#f6f3eb]'}`}>
                <div className="flex items-center gap-2.5">{t.icon}<span className="text-xs font-bold">{t.label}</span></div>
                {t.badge && <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-black ${activeTab===t.id ? 'bg-[#f3b13a] text-teal-950 font-bold' : 'bg-emerald-100 text-teal-950'}`}>{t.badge}</span>}
              </button>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-teal-900/10 shadow-sm p-5">
            ...
            {activeTab === 'general' && (
              <div className="space-y-4">
                <SectionHead icon={<Settings className="w-4 h-4 text-teal-950" />} title="General Settings" desc="Configure basic platform information" iconBg="bg-[#f6f3eb]" />
            ...
            {activeTab === 'security' && (
              <div className="space-y-4">
                <SectionHead icon={<Shield className="w-4 h-4 text-teal-950" />} title="Security Settings" desc="Protect your platform with advanced security features" iconBg="bg-[#f6f3eb]" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-teal-900/10">
            ...
            {activeTab === 'notifications' && (
              <div className="space-y-4">
                <SectionHead icon={<Bell className="w-4 h-4 text-teal-950" />} title="Notification Settings" desc="Manage how you receive notifications" iconBg="bg-[#f6f3eb]" />
                <div className="space-y-2">
                ...
                <div className="pt-3 border-t border-teal-900/10"><p className="text-xs font-black text-teal-900 mb-2">Notification Types</p>
                  <div className="space-y-2">
            ...
            {activeTab === 'users' && (
              <div className="space-y-4">
                <SectionHead icon={<Users className="w-4 h-4 text-teal-950" />} title="User Settings" desc="Configure user registration and permissions" iconBg="bg-[#f6f3eb]" />
                <div className="space-y-2">
            ...
            {activeTab === 'email' && (
              <div className="space-y-4">
                <SectionHead icon={<Mail className="w-4 h-4 text-teal-950" />} title="Email Configuration" desc="Configure SMTP settings for outgoing emails" iconBg="bg-[#f6f3eb]" />
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <Key className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-teal-600" />
                      <input type={showPassword?'text':'password'} value={email.smtpPassword} onChange={e=>{setEmail(em=>({...em,smtpPassword:e.target.value}));dirty();}} placeholder="••••••••" className="w-full pl-9 pr-9 py-2.5 text-sm rounded-xl border border-teal-900/10 bg-white text-teal-950 focus:outline-none focus:ring-2 focus:ring-teal-200 focus:border-teal-400" />
                      <button type="button" onClick={()=>setShowPassword(p=>!p)} className="absolute right-3 top-1/2 -translate-y-1/2 text-teal-600 hover:text-teal-800">{showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button>
                    </div>
                  </div>
                ...
                <div className="pt-3 border-t border-teal-900/10"><button className="flex items-center gap-2 px-4 py-2.5 text-sm font-bold rounded-xl bg-[#f6f3eb] border border-emerald-200 text-emerald-700 hover:bg-emerald-100 transition-colors"><Send className="w-4 h-4" />Send Test Email</button></div>
              </div>
            )}
            ...
            {activeTab === 'appearance' && (
              <div className="space-y-4">
                <SectionHead icon={<Palette className="w-4 h-4 text-teal-950" />} title="Appearance Settings" desc="Customize the look and feel of your platform" iconBg="bg-[#f6f3eb]" />
                <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-teal-900/10">
                  {[['Primary Color','primaryColor'],['Secondary Color','secondaryColor']].map(([l,k]) => (
                    <div key={k}><label className={labelCls}>{l}</label>
                      <div className="flex items-center gap-2">
                        <input type="color" value={(appear as any)[k]} onChange={e=>{setAppear(a=>({...a,[k]:e.target.value}));dirty();}} className="w-10 h-10 rounded-xl border border-teal-900/10 cursor-pointer p-0.5" />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-teal-900/10">
                  {[['Platform Logo','CF','bg-red-600'],['Favicon','?','bg-[#f6f3eb] text-teal-600']].map(([l,t,bg]) => (
                    <div key={l}><label className={labelCls}>{l}</label>
                      <div className="flex items-center gap-3">
                        ...
                        <button className="flex items-center gap-2 px-3 py-2 text-xs font-bold rounded-xl border border-teal-900/10 text-teal-900 hover:bg-[#f6f3eb] transition-colors"><Upload className="w-3.5 h-3.5" />Upload</button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {activeTab === 'database' && (
              <div className="space-y-4">
                <SectionHead icon={<Database className="w-4 h-4 text-teal-950" />} title="Database Management" desc="Backup and maintain your database" iconBg="bg-[#f6f3eb]" />
                <div className="grid grid-cols-3 gap-3">
                  {[{icon:<Database className="w-5 h-5 text-teal-950"/>,label:'DB Size',val:'2.4 GB',bg:'bg-[#f6f3eb] border-teal-900/10 text-emerald-900'},
                    {icon:<CheckCircle className="w-5 h-5 text-teal-950"/>,label:'Last Backup',val:'2 hrs ago',bg:'bg-[#f6f3eb] border-teal-900/10 text-emerald-900'},
                    {icon:<Zap className="w-5 h-5 text-teal-950"/>,label:'Total Records',val:'24,891',bg:'bg-[#f6f3eb] border-teal-900/10 text-emerald-900'}].map(s=>(
                    <div key={s.label} className={`p-4 rounded-xl border-2 ${s.bg}`}>{s.icon}<p className="text-[10px] font-bold text-teal-800 mt-2">{s.label}</p><p className="text-xl font-black mt-0.5">{s.val}</p></div>
                  ))}
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <button className="flex items-center justify-center gap-2 px-4 py-3 text-sm font-bold rounded-xl bg-[#f6f3eb] text-emerald-700 border border-emerald-200 hover:bg-emerald-100 transition-colors"><Download className="w-4 h-4" />Backup Database</button>
                  <button className="flex items-center justify-center gap-2 px-4 py-3 text-sm font-bold rounded-xl border border-teal-900/10 text-teal-900 hover:bg-[#f6f3eb] transition-colors"><Upload className="w-4 h-4" />Restore Backup</button>
                  <button className="flex items-center justify-center gap-2 px-4 py-3 text-sm font-bold rounded-xl border border-emerald-200 text-emerald-700 hover:bg-[#f6f3eb] transition-colors"><RefreshCw className="w-4 h-4" />Optimize DB</button>
                  <button className="flex items-center justify-center gap-2 px-4 py-3 text-sm font-bold rounded-xl border border-emerald-200 text-teal-950 hover:bg-[#f6f3eb] transition-colors"><Trash2 className="w-4 h-4" />Clear Cache</button>
                </div>
                <div className="pt-3 border-t border-teal-900/10"><p className="text-xs font-black text-teal-900 mb-2">Automatic Backups</p>
            ...
            {activeTab === 'advanced' && (
              <div className="space-y-4">
                <SectionHead icon={<Code className="w-4 h-4 text-teal-950" />} title="Advanced Settings" desc="Developer and system configuration" iconBg="bg-[#f6f3eb]" />
                ...
                <div className="space-y-4">
                  <div><label className={labelCls}>API Endpoint</label><input type="text" value="https://api.alumnex.com/v1" readOnly className={`${inputCls} font-mono bg-[#f6f3eb]`} /></div>
                  <div><label className={labelCls}>API Key</label>
                    <div className="flex items-center gap-2">
                      <input type="password" value="sk_live_51234567890abcdefghijk" readOnly className={`${inputCls} font-mono flex-1`} />
                       <button className="px-4 py-2.5 text-sm font-bold rounded-xl bg-[#f6f3eb] text-emerald-700 border border-emerald-200 hover:bg-emerald-100 transition-colors shrink-0">Regenerate</button>
                    </div>
                  </div>
                ...
                <div className="pt-3 border-t border-teal-900/10">
                  <button onClick={handleReset} className="flex items-center gap-2 px-4 py-2.5 text-sm font-bold rounded-xl bg-[#f6f3eb] border border-emerald-200 text-teal-950 hover:bg-emerald-100 transition-colors"><Trash2 className="w-4 h-4" />Reset All Settings to Default</button>
                </div>
              </div>
            )}
          </div>
